...
describe WoocommerceAPIV2::Order do
  it_behaves_like "a woocommerce resource"
  include_context "woocommerce_api_services", use_cassette: 'legacy/order'
  ...
  it "creates a new Order" do
    ...
    expect(wc_order.note).to eq "This is note"
  end

  it "updates attributes" do
    wc_order.update_attributes(customer_id: 3, note: "This new note")
    wc_order.reload
    expect(wc_order.customer_id).to eq 3
    expect(wc_order.note).to eq "This new note"
  end

  it "deletes an Order" do
    expect(wc_order.destroy).to include({"message"=>"Deleted order"})
  end
end
